...
(def #^{:doc "A character that contains the cell separator for each column in a row. 
              Default value: \\,"}
     *delimiter* \,)

(def #^{:doc "A string containing the end-of-line character for writing CSV files.
...
(defn- parse-csv-line 
  "Takes a CSV-formatted string or char seq (or something that becomes a char seq when seq is 
   applied) as input and returns a vector containing two values. The first is the first row 
   of the CSV file, parsed into cells (an array of strings). The second is the remainder of 
   the CSV file. Correctly deals with commas in quoted strings and double-quotes as 
   quote-escape in a quoted string."
  [csv-line]
  (let [csv-chars (seq csv-line)]
    (loop [fields []          ;; We're going to return this as the vector of fields. 
	   current-field []   ;; The working are for the cell we are working on. 
	   quoting? false     ;; Are we inside a quoted cell at this point in processing? 
      ...
      (cond (nil? current-char) (field-with-remainder nil)
	    ;; If we are on a newline while not quoting, then we can end this line and return. 
	    ;; Two cases for the different number of characters to skip. 
	    (and (not quoting?)
		 (lf? current-char)) (field-with-remainder remaining-chars)
	    (and (not quoting?)
		 (crlf? current-char remaining-chars)) (field-with-remainder (rest remaining-chars))
	    ;; If we see a comma and aren't in a quote, then end the current field and add to fields. 
	    (unquoted-comma? current-char) (recur (conj fields (apply str current-field))
						  []
						  quoting?
						  (first remaining-chars)
						  (rest remaining-chars))
	    (= \" current-char) (if (and (= \" (first remaining-chars))
					 quoting?)
				  ;; Saw "" so don't change quoting, just go to next character after "". 
				  (recur fields
					 (conj current-field \")
					 quoting?
				  ...
				  (recur fields
					 ...
					 (first remaining-chars)
					 (rest remaining-chars)))
	    ;; In any other case, just add the character to the current field and recur. 
	    true (recur fields
...
(defn- needs-quote? 
  "Given a string (cell), determine whether it contains a character that requires
   this cell to be quoted." 
  [cell]
  (or (s/contains? cell ",")
      (s/contains? cell "\"")
      (s/contains? cell "\n")
...
(deftest quoting
  ...
  (is (= [[",", "\"", ",,", ",,,"]] (parse-csv "\",\",\"\"\"\",\",,\",\",,,\"")))
  (is (= [["quoted", "\",\"", "comma"]] (parse-csv "quoted,\"\"\",\"\"\",comma"))))
